    ...
    def __init__(self, epsilon, mapData, unitData, ownFaction):
        ...
        for unit in unitData.units():
            if unit.faction == self.opforUnitProto.faction:
                ...
                self.sum += 1.0
        self.cull(unitData)
    def set_uniform_dist(self, mapData):
        setup_hexes = mapData.getSetupHexes(self.opforUnitProto.faction)
        self.distr = {}
        self.sum = 0.0
        for hex in mapData.hexes():
            self.distr[hex.id] = 0.0
        for hex in setup_hexes:
            self.distr[hex.id] = 1.0
            self.sum += 1.0
...
class AI:
    ...
    def process(self, message, response_fn=None):
        ...
        if msgD['type'] == "parameters":
            ...
            if self.mapData.hasSetupHexes():
                # Units may move anywhere in setup phase
                self.opforDistrib.set_uniform_dist(self.mapData)
            self.lastPhase = 0
